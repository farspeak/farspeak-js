...
(async () => {
  ...
  assert(getPegasusResult.id === entitiesProjectsResult.ids[1]);

  let updatedProjects = await farspeak.entity("projects").updateAll<Project>([
    {
      id: getPhoenixResult.id,
      title: "New Title",
      end_date: "2024-01-01",
    },
    {
      id: getPegasusResult.id,
  ...
  assert(updatedProjects[0]);
  assert(updatedProjects[1]);
  assert(!updatedProjects[2]);
  assert(updatedProjects[0].title === "New Title");
  assert(updatedProjects[0].end_date === "2024-01-01");
  assert(updatedProjects[1].description === "New Description");
  getProjectsResult = await farspeak
    .entity("projects")
    .getAll<Project, "projects">();
  console.log(getProjectsResult);
  ...
  assert(milestonesPhoenixResult.ids.length === 2);
  ...
  assert(milestonesPegasusResult.ids.length === 4);
