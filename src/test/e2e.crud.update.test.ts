...
(async () => {
  ...
  let updatedPhoenix = await farspeak.entity("projects").update<Project>({
    id: getPhoenixResult.id,
    title: "New Title",
    end_date: "2024-01-01",
  });
  assert(updatedPhoenix);
  assert(updatedPhoenix.title === "New Title");
  assert(updatedPhoenix.end_date === "2024-01-01");
